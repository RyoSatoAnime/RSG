...
function normalizeLoop(loop, base){
  if (!loop) return;
  // ★ デフォルトを '1-based' から '0-based' に変更
  const loopBase = base || '0-based'; 
  if (loopBase === '1-based'){
    if (typeof loop.start === 'number') loop.start = Math.max(0, loop.start - 1);
    if (typeof loop.end === 'number') loop.end = Math.max(0, loop.end - 1);
  }
}
...
const nowMs = ()=> (typeof performance!=='undefined' ? performance.now() : Date.now());
  ...
  class ClockScheduler {
    constructor(ctx, opts={}){
      this.ctx = ctx;
      this.lookaheadSec = opts.lookaheadSec ?? 0.18;
  ...
  class SQAudioEngine {
    ...
    playNote(params){
      ...
      if (!bus){
        if (params.autoCreateBus !== false){
          try{ this.createBus(busKey, { gain: 1.0 }); }catch(_){}
          bus = this.buses.get(busKey);
        }
    ...
    playSong(song, opts = {}){
      ...
      // loopBase は song 側の宣言を信じる
      normalizeLoop(loop, sg.loopBase);

      const tempo = opts.tempo ?? sg.tempo ?? 120;
      const tStart = opts.tStart ?? this.ctx.currentTime;
      const beatSec = 60 / Math.max(1, tempo);
      // --- Seek support (app-side BGM pause/resume) ---
      // opts.seekSec: start playback from this offset (in seconds).
      // For looped songs, the position is wrapped into the loop window.
      const seekSec = Math.max(0, opts.seekSec ?? 0);
      const seekBeatRaw = seekSec / beatSec;
      let startBeat = seekBeatRaw;

      if (loop && typeof loop.start==='number' && typeof loop.end==='number' && loop.end > loop.start){
        const loopLen = loop.end - loop.start;
        if (startBeat >= loop.end){
          // Wrap into [loop.start .. loop.end)
          startBeat = loop.start + ((startBeat - loop.start) % loopLen);
        }
        // Clamp negative/NaN safety
      ...
      // Auto-create buses referenced by the song (app-side boilerplate killer).
      // You can disable this by passing opts.autoCreateSongBuses === false.
      if (opts.autoCreateSongBuses !== false){
        const keys = new Set();
        for (const tr of (sg.tracks||[])){
          const k = tr && tr.busKey;
          if (k) keys.add(k);
          for (const ev of (tr.events||[])){
            const ek = ev && ev.busKey;
            if (ek) keys.add(ek);
          }
        }
        // Keep a small safety net for common channel names.
        keys.add('master'); keys.add('tri'); keys.add('squ'); keys.add('noi'); keys.add('wav');
        const defaultGain = (opts.defaultBusGain != null) ? opts.defaultBusGain : 0.90;
        for (const k of keys){
          if (!this.buses.get(k)){
            try{ this.createBus(k, { gain: defaultGain }); }catch(_){}
          }
        }
      }
      ...
      // --- One-shot schedule (with optional seek) ---
      if (loop && typeof loop.start==='number' && typeof loop.end==='number' && loop.end > loop.start){
        // 1) Schedule from startBeat up to loop.end once (this is the "current position -> loop boundary" segment).
        //    windowBeatBase=startBeat so that beat=startBeat maps to tStart.
        if (startBeat < loop.end){
          scheduleTracks(tStart, 0, {start: startBeat, end: loop.end}, startBeat);

          // 2) If there are events AFTER the loop end, schedule them once as well.
          //    Only if we haven't already passed loop.end due to seeking.
        ...
        // loopBaseT is the audioTime where the first repeat window starts (after reaching loop.end).
        const loopBaseT = tStart + Math.max(0, (loop.end - Math.min(startBeat, loop.end)) * beatSec);
        const loopLeadSec = opts.loopLeadSec ?? 6.0;
        let nextLoopI = 0;
        ...
        // Prime the first repeat window early.
        scheduleTracks(loopBaseT, 0, {start: loop.start, end: loop.end}, loop.start);
        nextLoopI = 1;

        pumpLoop();
        loopTimer = setInterval(pumpLoop, Math.max(20, this.sched.intervalMs));
      } else {
        // No loop: schedule from startBeat onward once.
        scheduleTracks(tStart, 0, {start: startBeat, end: Number.POSITIVE_INFINITY}, startBeat);
      }
  ...
  function normalizePack(pack){
    // Accepts various shapes and returns a canonical pack:
    // { song, toneBank:{meta,tones}, waveBank:{meta,waves} }
    const p = pack || {};
    const tb = p.toneBank || {};
    const wb = p.waveBank || {};
    const toneBank = {
      meta: (tb.meta && typeof tb.meta === 'object') ? tb.meta : {},
      tones: (tb.tones && typeof tb.tones === 'object') ? tb.tones : {},
    };
    const waveBank = {
      meta: (wb.meta && typeof wb.meta === 'object') ? wb.meta : {},
      waves: (wb.waves && typeof wb.waves === 'object') ? wb.waves : {},
    };
    return { song: p.song || null, toneBank, waveBank };
  }

  function loadPack(engine, pack){
    const p = normalizePack(pack);
    if (!engine) throw new Error('engine required');
    engine.loadBanks({ toneBank: p.toneBank, waveBank: p.waveBank });
    return p.song;
  }

  const SQAE = {
    version: 'vNext_hardline_v0.3',
